...
Before anything else, silently check if the current project already has components installed (e.g., `components/vital-signs/`, `app/ehr/`, etc.):
...
| **ehr** | patient record, medical history, clinical notes, evolution, health record, EHR, EMR, patient file | Read and execute `${CLAUDE_PLUGIN_ROOT}/skills/ehr/SKILL.md` |
