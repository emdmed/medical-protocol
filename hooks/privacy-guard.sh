...
# Only activate in doctor projects (where the protocol is installed as CLAUDE.md).
# Skip for the CDN source repo itself and any project without the protocol.
if [ ! -f "$PROJECT_DIR/.claude/CLAUDE.md" ]; then
  exit 0
fi
# Extra check: the CLAUDE.md should contain the medical protocol marker
if ! head -5 "$PROJECT_DIR/.claude/CLAUDE.md" | grep -qi 'medical protocol'; then
  exit 0
fi
